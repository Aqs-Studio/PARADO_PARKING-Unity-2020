...
public class ColorPicker : MonoBehaviour {
	...
	void Start()
	{
        ...
        for (int i = 0; i < size; i++)
        {
            if (receiver[i])
            {
                receiver[i].SendMessage(colorGetFunctionName, this, SendMessageOptions.DontRequireReceiver);
            }
        }
	}
		...
		if(mState == ESTATE.Showed)
		{
			...
			if(GUI.Button(new Rect(startPos.x + sizeCurr + 10, startPos.y + 150, 60, 20), "Apply"))
			{
                ...
                for (int i = 0; i < size; i++)
                {
                    if (receiver[i])
                    {
                        receiver[i].SendMessage(colorSetFunctionName, SelectedColor, SendMessageOptions.DontRequireReceiver);
                    }
                }
			}
